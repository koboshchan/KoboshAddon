	...
	implements UpdateListener, RenderListener, GUIRenderListener
{
	...
	private void renderNBTWindow(GuiGraphics context)
	{
		if(nbtData == null)
			return;
		...
		context.drawString(tr, "NBT Data Viewer (Press ESC to close)",
		...
		for(int i = 0; i < Math.min(lines.length, maxLines); i++)
		{
			...
			context.drawString(tr, line, windowX + 5, contentY + i * 10,
				0xFFFFFFFF, false);
		}
		...
		if(lines.length > maxLines)
		{
			...
			context.drawString(tr, scrollText, windowX + 5,
